<template>
  <q-uploader class="q-mt-lg" ref="archivo" :factory="tipoSubida === 'nueva' ? factoryFnPost : factoryFnPut" accept=".pdf, .doc, .docx, .ppt, .pptx, .xls, .xlsx, .csv, .odt, .ods, .odp, .rtf, .txt, image/*, video/*, audio/*" color="primary" field-name="file"
    ...
    <template v-slot:header="scope">
      <div class="row no-wrap items-center q-pa-sm q-gutter-xs">
        <q-btn v-if="scope.queuedFiles.length > 0" dense flat icon="clear_all"
          ...
          <q-tooltip class="bg-secondary">Borrar todos</q-tooltip>
        </q-btn>
        <q-btn v-if="scope.uploadedFiles.length > 0" dense flat icon="done_all"
               round @click="scope.removeUploadedFiles">
          <q-tooltip class="bg-secondary">Quitar subidos</q-tooltip>
        </q-btn>
        ...
        <q-btn v-if="scope.canAddFiles" dense flat icon="add_box" round type="a"
          ...
          <q-tooltip class="bg-secondary">Elegir archivos</q-tooltip>
        </q-btn>
        <q-btn v-if="scope.canUpload" dense flat icon="cloud_upload" round
               @click="scope.upload">
          <q-tooltip class="bg-secondary">Subir archivos</q-tooltip>
        </q-btn>

        <q-btn v-if="scope.isUploading" dense flat icon="clear" round
               @click="scope.abort">
          <q-tooltip class="bg-secondary">Cancelar subida</q-tooltip>
        </q-btn>
      </div>
    </template>
  </q-uploader>
...
function factoryFnPut() {
  return {
    url: `${urlApi}/${props.nombreArchivoAct}`,
    method: 'PUT',
  }
}
